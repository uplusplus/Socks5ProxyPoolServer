...
class Resquest(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.send_header("Content-type","text/plain")  #设置服务器响应头
        self.send_header("format","ip:port")     #设置服务器响应头
        self.end_headers()

        proxy = random.choice(g_proxys)
        self.wfile.write(proxy.encode())  #里面需要传入二进制数据，用encode()函数转换为二进制数据   #设置响应body，即前端页面要展示的数据
